"""Pydantic params/result models for Brex Connector.
...
class ConnectionScoped(BaseModel):
    connection_id: str = Field(
        "",
        description="Which connected Brex account to use (see list_connections). Omit if only one is connected.",
    )


# ──────────────────────────────────────────────────────────────────────────
# Connection -- static admin-generated user token, no OAuth
# ──────────────────────────────────────────────────────────────────────────


class ConnectBrexParams(BaseModel):
    user_token: str = Field("", description="Your Brex user token (dashboard.brex.com > Developer Settings).")
    label: str = Field("", description="Optional friendly label for this connection, e.g. 'Acme Inc Brex'.")
...
class DisconnectBrexParams(BaseModel):
...
class ListEntitiesParams(ConnectionScoped):
    entity: str = Field(description="Resource type: cards, transactions, users, expenses, budgets, departments, locations.")
    filter_expr: str = Field("", description="Optional query-string filter supported by the resource (e.g. 'user_id=...').")
...
class GetEntityParams(ConnectionScoped):
    ...
    record_id: str = Field(description="The record's Brex id.")
...
class CreateExpenseParams(ConnectionScoped):
    user_id: str = Field(description="The Brex user id this expense is for.")
    amount: float = Field(description="Expense amount.")
    currency: str = Field("USD", description="Three-letter currency code.")
    memo: str = Field("", description="Optional memo/description for the expense.")


class UpdateUserParams(ConnectionScoped):
    user_id: str = Field(description="The Brex user id to update.")
...
class SetCardStatusParams(ConnectionScoped):
    card_id: str = Field(description="The Brex card id.")
...
class GetCardUtilizationReportParams(ConnectionScoped):
    limit: int = Field(100, ge=1, le=200, description="Number of cards to scan for the report.")
